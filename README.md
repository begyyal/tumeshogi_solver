...
- **[WEB版](https://begyyal.net/tss)は後述のスペックで動作していません。**  
  ...
  ちなみにLambdaのCPU性能はメモリ量に比しており、前述の性能だと処理速度的には後述のベンチマークをある程度下回ることを確認しています。

- 難度相応にメモリ容量も要するため、メモリが不足している場合はOOMで落ちる可能性があります。  
アプリの処理に割り当てるメモリ容量はjavaコマンド実行時の引数等で指定が可能です。(`-Xmx`など)  
詳しくは[Javaのドキュメント](https://docs.oracle.com/en/java/javase/15/docs/specs/man/java.html#extra-options-for-java)をご確認ください。
