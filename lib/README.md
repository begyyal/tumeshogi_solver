...
詰み筋が存在する場合はその手筋を出力します。  
[出力フォーマット](#出力内容)および[ビルド形式](#導入)は用途によって切替が可能です。
...
## 実行形式(`gradle_exe.build`)

javaのjarファイルを直接実行する形式になります。

1. Java15以上の実行環境を用意してください。  
    - https://www.oracle.com/java/technologies/downloads/  
2. コンソールにてカレントディレクトリを本リポジトリのルートへ移してください。
3. ビルドします。(2回目以降は不要)  
    - `./gradlew build -b ./build_exe.gradle` 
    - windowsのコマンドプロンプトの場合は上記コマンド頭の`./`を外してください
4. 生成されたjarファイルにて以下の要領でプログラムを実行します。
    - `java -jar ./build_exe/libs/ts_solver.jar -t 1 31xfz11yb12yh xe1ya18b3c4d4e3f1g2`
    - [引数](#引数のフォーマット)および[出力内容](#出力内容)の詳細は後述の項をご参照ください。

## ライブラリ(`gradle.build`)

jarのライブラリ形式になります。
- ビルド手順は前述同様のgradleビルドです
ビルドファイルは`gradle.build`のため、`-b`以降のファイル指定は不要です。

APIおよびIFは以下の通りですが、基本的に実行形式と同様です。

- API : `begyyal.shogi.TsSolver#exe(String args[])`のみです。
- 入力 : [こちら](#引数のフォーマット)の内容での文字列配列となります。
- 出力 : [こちら](#出力内容)の内容での文字列配列となります。
